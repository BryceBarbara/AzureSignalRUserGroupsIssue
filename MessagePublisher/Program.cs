        ...
        private static async Task StartAsync(MessagePublisher publisher)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    {
	                    var task = publisher.ManageUserGroup(args[1], args[2], args[3]);
	                    if (args[1] == "check" && task is Task<bool> boolTask)
	                    {
		                    var result = await boolTask;
		                    Console.WriteLine($"check is user '{args[2]}' in group '{args[3]}': {result}");
	                    }
	                    else
	                    {
